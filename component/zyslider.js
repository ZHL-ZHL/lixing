...
Component({
  ...
  ready: function () {
    let that = this;
    const getSystemInfo = util.default.wxPromisify(wx.getSystemInfo)
    const queryContainer = util.default.wxPromisify(wx.createSelectorQuery().in(this).select(".container").boundingClientRect)
    util.default.wxPromisify(wx.getSystemInfo)()
      .then(res => {
